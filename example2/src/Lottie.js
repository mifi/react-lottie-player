...
import lottieApi from 'lottie-api/dist/lottie_api.js'
...
const Lottie = memo(({
  ...
  useEffect(() => {
    ...
    animApiRef.current = lottieApi.createAnimationApi(animRef.current)
  ...
  useEffect(() => {
    ...
    properties.forEach((property) => {
      const propertyPath = animApiRef.current.getKeyPath(property);
      const value = animationControl[property];
      animApiRef.current.addValueCallback(propertyPath, () => value);
    });
  }, [animationControl])
